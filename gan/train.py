...
def build_transforms():
    ...
    ds_transforms = transforms.Compose(
        [transforms.ToTensor(),
        transforms.Lambda(lambda resize: resize * 2 - 1)]
    )
...
def get_optimizers_and_schedulers(gen, disc):
    ...
    # ipdb.set_trace()
    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(optim_discriminator, lr_lambda=lambda x: 1 - x / 500000)
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(optim_generator, lr_lambda=lambda x: 1 - x / 100000)
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                output_generator = gen(train_batch.shape[0])
                output_discriminator_train = disc(train_batch)
                output_discriminator_gen = disc(output_generator.detach())

                # TODO: 1.5 Compute the interpolated batch and run the discriminator on it.

                discriminator_loss = disc_loss_fn(
                    output_discriminator_train,
                    output_discriminator_gen,
                    None, None, None
                )

            optim_discriminator.zero_grad(set_to_none=True)
            # ipdb.set_trace()
            discriminator_loss = discriminator_loss.half()
            # ipdb.set_trace()
            ...
            if iters % 5 == 0:
                with torch.cuda.amp.autocast(enabled=amp_enabled):
                    ...
                    output_generator = gen(train_batch.shape[0])
                    output_discriminator_gen = disc(output_generator)
                    generator_loss = gen_loss_fn(output_discriminator_gen)
                    print("Generator loss: ", generator_loss, "Discriminator loss: ", discriminator_loss)
